...
Train_Method = 'Supervised'      # Supervised    AutoML
...
class RS_MLP(nn.Module):
    ...
    def forward(self, u_weight, m_weight, userId, movieId, movie_vec):
        ...
        movie_emb = self.emb_movie(movieId)
        ...
        v_movie = sum([torch.reshape(m_weight[:, i], (len(m_weight), -1)) * self.tanh(self.bn_movie(self.W_movie[i](movie_emb[:,Emb_Split[i]:Emb_Split[i+1]]))) for i in range(len(Emb_Size))])
...
def update_controller(index, features, target):
    ...
    if Train_Method == 'AutoML' and index > 0:
        ...
        new_mw = movie_policy(moviePop, old_mw, old_ml) 
...
def update_RS(index, features, Len_Features, target, mode):
    ...
    movie_weights[batch_train[:, 1], :] = new_mw.detach().cpu().numpy()
