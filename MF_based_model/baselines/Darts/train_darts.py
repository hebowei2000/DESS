...
class User_weights(nn.Module):
    def __init__(self, user_num, emb_sizes):
        super().__init__()
        initial = np.ones((user_num, len(emb_sizes)), dtype=np.float32) / len(emb_sizes)
        self.weights = nn.Parameter(torch.from_numpy(initial), requires_grad=True)
        self.softmax = nn.Softmax(dim=1)

    def forward(self, userid):
        return self.softmax(self.weights[userid])

class Movie_weights(nn.Module):
    def __init__(self, movie_num, emb_sizes):
        super().__init__()
        initial = np.ones((movie_num, len(emb_sizes)), dtype=np.float32) / len(emb_sizes)
        self.weights = nn.Parameter(torch.from_numpy(initial), requires_grad=True)
        self.softmax = nn.Softmax(dim=1)

    def forward(self, movieid):
        return self.softmax(self.weights[movieid])
...
class RS_MLP(nn.Module):
    ...
    def forward(self, u_weight, m_weight, userId, movieId, movie_vec):
        ...
        # print(v_user.dtype, v_movie.dtype)

        return torch.einsum("nd, nd->n", v_user, v_movie)


def update_weights(index, features, target):
    ...
    uw = user_weights(userId)
    """
    if index % 50000 == 0:
        print("uw: ", uw[:10])
    """
    mw = movie_weights(movieId) if Dy_Emb_Num == 2 else 0

    rating = model(uw, mw, userId, movieId, movie_vec)
    #rating = rating.squeeze(1).squeeze(1) if Loss_Type == 'CrossEntropyLoss' else rating.squeeze(1)
    # print('new_uw', new_uw[0], '\nnew_mw', new_mw[0] if Dy_Emb_Num == 2 else 0, '\nrating', rating[0])
...
if __name__ == "__main__":
    ...
    user_weights = User_weights(User_Num, Emb_Size)
    movie_weights = Movie_weights(Movie_Num, Emb_Size)
    model = RS_MLP(Output_Dim, Dy_Emb_Num)
    user_weights.to(device)
    movie_weights.to(device)
    model.to(device)
    if Model_Gpu:
        print('\n========================================================================================\n')
        print('Model_Gpu?:', next(model.parameters()).is_cuda, next(user_weights.parameters()).is_cuda, next(movie_weights.parameters()).is_cuda)
    ...
    optimizer_user  = torch.optim.Adam(filter(lambda p: p.requires_grad, user_weights.parameters()), lr=LR_darts, weight_decay=0)
    optimizer_movie = torch.optim.Adam(filter(lambda p: p.requires_grad, movie_weights.parameters()), lr=LR_darts, weight_decay=0)
    optimizer_darts = torch.optim.Adam(filter(lambda p: p.requires_grad, list(user_weights.parameters()) + list(movie_weights.parameters())), lr=LR_darts, weight_decay=0)
    losses = {'train': [], 'test': []}
    ...
    for epoch_i in range(Epoch):
        ...
        while index < Len_Train_Features:
            ...
            if index > 0:
                update_weights(index, train_features, train_target)
    ...
    while index < Len_Test_Features:
        ...
        if index > 0:
            update_weights(index, test_features, test_target)
    ...
    feature_data.to_csv('./results/feature_data_with_loss_{}_Darts_{}_{}.csv'.format(Dy_Emb_Num, Loss_Type, DATA_SET), index=None)
    ...
    if Dy_Emb_Num == 1:
        ...
        pd.DataFrame(result_user).to_csv('./results/result_{}_Darts_{}_{}_user.csv'.format(Dy_Emb_Num, Loss_Type, DATA_SET), index=None,
                                          header=Head)
        ...
        Head = list(feature_data.columns) + ['count']
        pd.DataFrame(result_movie).to_csv('./results/result_{}_Darts_{}_{}_movie.csv'.format(Dy_Emb_Num, Loss_Type, DATA_SET), index=None,
                                          header=Head)
    ...
    Head = list(feature_data.columns) + ['count']
    pd.DataFrame(result).to_csv('./results/result_{}_Darts_{}_{}_trendency.csv'.format(Dy_Emb_Num, Loss_Type, DATA_SET), index=None, header=Head)
