...
def update_ladder_bandit_controller(index, features, target, Setting_Movie_Popularity, Setting_User_Popularity):
    ...
    if Train_Method == 'AutoML' and index > 0:
        ...
        user_adj_test = mask * (1 - user_adj_prediction)
        ...
        if Dy_Emb_Num == 1:
            ...
            rewards_u_test = (rewards_u_test > threshold).int()            
            most_rewards_u = torch.max(rewards_u, rewards_u_test)
            for i in range(len(rewards)):
                regret.append(regret[-1] + (most_rewards_u[i] - rewards_u[i]).item())
...
def update_RS_New(index, features, Len_Features, target, mode, Setting_Movie_Popularity, Setting_User_Popularity):
    ...
    old_ul = torch.tensor(user_losses[batch_train[:, 0], :], requires_grad=False).to(device)
    old_ml = torch.tensor(movie_losses[batch_train[:, 1], :], requires_grad=False).to(device)
    movie_vec = torch.tensor([genome_scores_dict[str(batch_train[:, 1][i])] for i in range(len(batch_train[:, 1]))], requires_grad=False).to(device) if Dy_Emb_Num == 1 else None
    ...
    with torch.no_grad():
        ...
        ###Linear UCB
        user_policy_context =  torch.reshape(userPop, (-1, 1))
        new_uw_prediction = user_policy.predict(user_policy_context.detach().cpu().numpy())
        new_uw = torch.tensor(new_uw_prediction, requires_grad=False).to(device)


        if Dy_Emb_Num == 2:
            #old_mw_one_hot = nn.functional.one_hot(old_mw, num_classes=len(Emb_Size))
            #movie_policy_context = torch.cat((old_mw_one_hot, moviePop/max_movie_popularity), 1)
            movie_policy_context = torch.reshape(moviePop, (-1, 1))
            new_mw_prediction = movie_policy.predict(movie_policy_context.detach().cpu().numpy())
            new_mw = torch.tensor(new_mw_prediction, requires_grad=False).to(device)
        else:
            new_mw = 0
...
def update_RS_Old(index, features, Len_Features, target, mode, Setting_Movie_Popularity, Setting_User_Popularity):
    ...
    with torch.no_grad():
        ...
        user_adj_prediction_test = 1 - user_adj_prediction
        user_adj_test = mask * user_adj_prediction_test
        new_uw_test = old_uw + user_adj_test
...
if __name__ == "__main__":
    ...
    save_regret_name = './save_regret_true/MEDIA_DyEmbNum{}_LossType{}_Reward_Base{}_last{}_TestAcc{:.4f}_Dataset{}_regret.pkl'.format(
            Dy_Emb_Num, Loss_Type, Reward_Base, last_num,
            correct_num / test_num, DATA_SET)
    save_pseudo_regret_name = './save_pseudo_regret_true/MEDIA_DyEmbNum{}_LossType{}_Reward_Base{}_last{}_TestAcc{:.4f}_Dataset{}_pseudo_regret.pkl'.format(
            Dy_Emb_Num, Loss_Type, Reward_Base, last_num,
            correct_num / test_num, DATA_SET)
