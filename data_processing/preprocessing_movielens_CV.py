...
# ml-latest: rating_csv:rating:27753444  user:283228  movie: 53889  || Experimental data: merged_user: 283099 merged_movie: 13176 merged_ratings:27417362 (Train 22000000 + Test 5000000) 
# ml-20M: rating_csv:ratings:20000263 user:138493 movie:26744 || Experimental data:  merged_user:138493 merged_movie: 10370 merged_ratings:19800443 (Train 16000000 + Test 3800000)
# ml-25M：
...
print('Number of movies in both genome_scores.csv and ratings.csv: {}. Take up {}% of ratings.csv'.format(number, round(number/26744*100)))
...
#movie_POD_list = list()
index = 0
for line_user, line_movie in zip(train_features[:,0], train_features[:,1]):
    if line_user in user_count_dict:
        user_count_dict[line_user] += 1
        user_rating_count.append([user_count_dict[line_user]])
    else:
        user_count_dict[line_user] = 1
        user_rating_count.append([1])
    if line_movie in movie_count_dict:
        movie_count_dict[line_movie] += 1
        movie_rating_count.append([movie_count_dict[line_movie]])
    else:
        movie_count_dict[line_movie] = 1
        movie_rating_count.append([1])
    if line_user in user_history:
        user_history[line_user].append(line_movie)
    else:
        user_history[line_user] = [line_movie]
    #if line_movie in movie_history:
    #    movie_history[line_movie].append(line_user)
    #else:
    #    movie_history[line_movie] = [line_user]
    Interest = sum([np.array(genome_scores_dict[str(m_id)]) for m_id in user_history[line_user]]) / len(user_history[line_user]) 
    #Interest_vector[str(line_user)] = Interest
    user_IND_list.append([sum([np.linalg.norm(np.array(genome_scores_dict[str(m_id)]) - Interest) for m_id in user_history[line_user]]) / len(user_history[line_user])])
    #Popularity = sum([Interest_vector[str(u_id)] for u_id in movie_history[line_movie]]) / len(movie_history[line_movie]) 
    #movie_POD_list.append([sum([np.linalg.norm(np.array(Interest_vector[str(u_id)]) - Popularity) for u_id in movie_history[line_movie]]) / len(movie_history[line_movie])])
    index += 1
    if index % 1000 == 0: print('Generation round', index)
...
train_feature_data['user_IND'] = pd.DataFrame(user_IND_list)
#train_feature_data['movie_PND'] = pd.DataFrame(movie_POD_list)
train_feature_data.to_csv('./{}/{}_train_feature_data_CV_{}_{}.csv'.format(DATA_PATH, DATA_SET_NAME, Train_Size, Output_Dim), index = None)
print('train_feature_data_CV_{}_{}.csv done.\n'.format(Train_Size, Output_Dim))
...
#movie_POD_list = list()
index = 0
for line_user, line_movie in zip(test_features[:,0], test_features[:,1]):
    if line_user in user_count_dict:
        user_count_dict[line_user] += 1
        user_rating_count.append([user_count_dict[line_user]])
    else:
...
test_feature_data['user_IND'] = pd.DataFrame(user_IND_list)
#test_feature_data['movie_PND'] = pd.DataFrame(movie_POD_list)
test_feature_data.to_csv('./{}/{}_test_feature_data_CV_{}_{}.csv'.format(DATA_PATH, DATA_SET_NAME, len(test_feature_data), Output_Dim), index = None)
print('test_feature_data_CV_{}_{}.csv done.\n'.format(len(test_feature_data),Output_Dim))
...
print('\ntest_features\n',  test_features,  type(test_features),  test_features.shape)
print('\ntest_target\n',    test_target,    type(test_target),    test_target.shape)
pickle.dump((train_features, test_features, train_target, test_target), open('./{}/{}_TrainTest_CV_{}_{}.data'.format(DATA_PATH, DATA_SET_NAME, Train_Size, Output_Dim), 'wb'))
